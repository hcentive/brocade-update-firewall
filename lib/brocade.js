...
var brocade = JSON.parse(fs.readFileSync(path.join('.', 'conf', 'brocade.json')));
...
var backupProtectionClass = function(protectionClass, callback) {
  ...
  var req = https.request(options(protectionClass, 'GET'), (res) => {
    ...
    res.on('end', () => {
      ...
      createProtectionClass(brocade.protection_class_backup, body, callback);
    });
  });
...
exports.updateBannedIPs = function(protectionClass, addresses, callback) {
  ...
  logger.info("Updating " + protectionClass + " with " + addresses.length + " CIDR blocks");
    ...
    if (error) {
      ...
      var req = https.request(options(protectionClass, 'PUT'), (res) => {
        ...
        res.on('end', () => {
          ...
          callback(null, "Updated " + protectionClass + " with " + addresses.length + " CIDR blocks");
        });
      });
