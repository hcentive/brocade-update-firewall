...
var options = function (protectionClass, m, clength){
  var contentLength = clength || 0;
  return {
...
exports.updateBannedIPs = function(protectionClass, addresses, callback) {
  ...
  createBannedPayload(addresses, protectionClass, function(error, results) {
    if (error) {
      ...
      var body = "";
      var req = https.request(options(protectionClass, 'PUT', Buffer.byteLength(JSON.stringify(results))), (res) => {
        res.on('data', (d) => {
          body += d;
        });
