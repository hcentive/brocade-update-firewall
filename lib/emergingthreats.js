...
exports.getAddresses = function(url, callback) {
  threaturl = url || threatsURL;
  var ranges = [];
  https.get(threaturl, function (response) {
    ...
    reader.on('close', function () {
      console.log(ranges.length + ' address ranges read from ' + threaturl);
      callback(null, ranges);
    });
  }).on('error', function (err) {
    console.error('Error downloading ' + threaturl, err);
    callback(err);
  });
};
